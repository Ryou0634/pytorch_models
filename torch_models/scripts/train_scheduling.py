...
go_up_dict = {'loss': False, 'accuracy': True, 'BLEU': True, 'sent_BLEU': True}
eval_dict = {'loss': EvaluatorLoss, 'accuracy': EvaluatorC,
             'BLEU': EvaluatorSeq, 'sent_BLEU': EvaluatorSeq}
...
def train_lr_decay(model, train_loader, dev_loader, measure,
                   ...
                   save_path=None, tenacity=1):

    # Preparing
    trainer = Trainer(model, train_loader)
    dev_evaluator = eval_dict[measure](model, dev_loader, measure=measure)
    score_monitor = ScoreMonitor(tenacity=tenacity, go_up=go_up_dict[measure])
